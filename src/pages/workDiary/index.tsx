...
const WorkDiaryPage: React.FC = () => {
  ...
  const { userId , roleId} = useTypedSelector((state) => state.auth);
  const { data, loading, error } = useTypedSelector((state) => state.getTask);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="bg-white shadow-lg rounded-lg p-6 mb-8">
        <h1 className="text-xl mb-0 font-bold text-gray-500">Work Diary</h1>
        <Formik
        ...
        <div className="overflow-x-auto max-h-96">
          <table className="w-full border-collapse bg-white rounded-lg shadow-md min-w-[800px]">
            <thead>
            ...
            <tbody>
              {data.length > 0 ? (
                data.map((task: any, index: number) => (
                  ...
                  >
                    <td className="py-3 px-4 text-gray-700">{task.id}</td>
                    ...
                    <td className="py-3 px-4 text-gray-700">
                    {task.status === 0 ? "In Progress" : "Completed"}                      </td>
